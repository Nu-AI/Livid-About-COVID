...
class preprocess_new_virus_data:
    ...
    def create_nday_list(self, n, data_list):
        n_day_list = [int(data_list[i + n]) - int(data_list[i]) for i in range(len(data_list) - n)]
        for j in range(n):
    ...
    def create_csv(self, full_key_list,pop_list,aggregated_confirmed,active_confirmed,recovered,death):
        ...
        for i in range(len(full_key_list)):
            ...
            case_dict['aggregated_confirmed'] = aggregated_confirmed[i]   # agg_numpyy[i]/int(pop_list[i])
            agg_numpy = aggregated_confirmed[i].astype(int)
            rec_numpy = recovered[i].astype(int)
            dea_numpy = death[i].astype(int)
            ...
            one_day_change[full_key_list[i]] = self.clear_list(list(self.create_nday_list(1,temp)))

            three_day_change[full_key_list[i]] = self.clear_list(list(self.create_nday_list(3,temp)))

            seven_day_change[full_key_list[i]] = self.clear_list(list(self.create_nday_list(7,temp)))
            ...
            print(aggregated_confirmed[i].shape, "**************", aggregated_confirmed[i][0])
            active_dict[full_key_list[i]] = self.clear_list(list(actual_active_confirmed))
            recovered_dict[full_key_list[i]] = self.clear_list(list(recovered[i]))
    ...
    def save_csv(self, orig_dict, *args):
        csv_name_list = ['Full','Total_cases', 'Active_cases', 'Recovered', 'Death', 'One-day-change', 'Three-day-change', 'Seven-day-change']
        count  = 0
        for arg in args:
            ...
            if (csv_name_list[count] == 'Total_cases' or csv_name_list[count] == "Death" or csv_name_list[count] == 'Recovered'):
...
virus_cities = rd.preprocess_virus_data()
new_list,dict = virus_cities.create_keylist('Data319.xlsx')
...
population_list = [filtered_list[i] for i in range(len(filtered_list)) if i%2==1]

new_list= [new_list[i].split("-",1)[0] for i in range(len(new_list))]
print (new_list, "\n",len(new_list))
virus_methods = preprocess_new_virus_data(new_list)   # Edit for whichever area you need the data for.
key_list, dict, orig_dict = virus_methods.create_keylist_path(urllib.request.urlopen("http://hgis.uw.edu/virus/assets/virus.csv"))
updated_dict = virus_methods.select_keys(keylist= key_list,dict = dict)
aggregated_confirmed,active_confirmed,recovered,death = virus_methods.format_data(updated_dict)     # Obtain the final np arrays.
country_dict, total_dict,active_dict, recovered_dict, death_dict, one_day_dict, three_day_dict, seven_day_dict = virus_methods.create_csv(new_list,population_list,aggregated_confirmed,active_confirmed,recovered,death)

print (total_dict.keys())
virus_methods.save_csv(orig_dict,country_dict,total_dict,active_dict, recovered_dict, death_dict, one_day_dict, three_day_dict, seven_day_dict)
