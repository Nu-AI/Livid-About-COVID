...
class DataRetriever(object):
    ...
    def fill_missing_days_df(self, df_required):
        ...
        actual_days_list = list(
            pd.to_datetime(np.arange(1, total_no_days + 1, 1), unit='D',
                           origin=pd.Timestamp(df_required['date'][
                                                   df_required.index[
                                                       0]])).astype(str))
    ...
    def extend_required_df(self, df_required):
        ...
        temp = list(
            pd.to_datetime(
                ...
                origin=pd.Timestamp(df_required['date'][df_required.index[-1]])
            ).astype(str)
        )

        index = 0 - no_days
        for i in range(no_days):
            df_required = df_required.append(pd.Series([np.NaN]),
                                             ignore_index=True)
        ...
        for keys in df_required:
            ...
            if df_required[keys].nunique() == 1:
                if keys in keylist:
                    ...
                    df_required[keys] = df_required[keys].fillna(
                        list(df_required[keys].unique())[0])
    ...
    def apply_extension(self, df_required, region):
        ...
        for county in unique_list:
            # print(county)
            df = self.extend_required_df(
                df_required[df_required[region] == county])
    ...
    def get_mobility_data(self):
        ...
        df = pd.read_csv(urllib.request.urlopen(
            "https://www.gstatic.com/covid19/mobility/Global_Mobility_Report.csv"),
            low_memory=False)
        # Lambda function to filter the required data from the global mobility data.
        filtering_func = lambda x, y: x.where(x['sub_region_2'].isin(
            y) == True).dropna().reset_index() if y == self.counties else \
        ...
        if self.country is not None and self.states is None:
            df_country = df[df['country_region'] == self.country].dropna(
                how='all').reset_index()
            ...
            if self.counties is not None:
                if 'all' not in self.counties:
                    ...
                    df_required = self.apply_extension(df_required,
                                                       'sub_region_2')
            else:
            ...
            df_state = df.where(
                df['sub_region_1'].isin(self.states) == True).dropna(
                how='all').fillna('').reset_index()
            # The state total mobility
            if self.counties is None:
                df_required = df_state[
                    df_state['sub_region_2'] == ''].reset_index()
    ...
    def get_population_data(self, df_required):
        ...
        url_pop_fmt_str = ("https://www2.census.gov/programs-surveys/popest/"
                           "tables/2010-2019/counties/totals/"
                           "co-est2019-annres-{}.xlsx")
        base_path = [url_pop_fmt_str.format(LUT_dict[state])
        ...
        for paths in base_path:
            ...
            pop_df = pop_df[['Geographic Area',
                             'Unnamed: 12']].iloc[1:].reset_index()
            ...
            get_state_arr = lambda state_list, pop_df: \
                [state_list[count]] * len(pop_df['Geographic Area'].tolist())
            # Filter out the data required for the counties
            if self.counties is not None:
                pop_df = pop_df.where(
                    pop_df['Geographic Area'].isin(
                        df_required[
                            df_required['sub_region_1'] == state_list[count]
                            ]['sub_region_2'].unique()) == True
            ...
            # Just get the population for the required state
                ...
                pop_df = pop_df.where(
                    pop_df['Geographic Area'] == state_list[i]).dropna(
                    how='all').reset_index()
                pop_df['State'] = get_state_arr(state_list, pop_df)
                count += 1
    ...
    def get_cases_data(self, df_required):
        ...
        state_cases_update = lambda state_cases_df: state_cases_df[
            state_cases_df['state'].isin(self.states) &
            (state_cases_df['date'].isin(df_required['date'].unique().tolist()))
            ]
        ...
        if self.counties is not None:
            ...
            for county in unique_counties:
                ...
                required_cdf = df_required[
                    df_required['sub_region_2'] == county_name]
                county_df = county_df[county_df['date'].isin(
                    required_cdf['date'].unique().tolist())]
            ...
            # For selected counties in the state
                ...
                new_counties = [" ".join(county.split(" ")[:-1]) for county in
                                self.counties]
        ...
        # If require only state data
            ...
            return state_cases_df.sort_values(
                by=['state', 'date']).reset_index()
    ...
    def reorganize_case_data(self, df_required, df_county_cases):
        ...
        for county in new_counties:
            if self.counties is not None:
                # Check if the case data for counties required are present in
                # the case dataframe
                if county in all_case_counties:
                    temp_df = df_county_cases[
                        df_county_cases['county'] == ' '.join(
                            county.split(" ")[:-1])]
            ...
            if len(temp_df['cases'].tolist()) < length:
                ...
                new_temp_df['state'] = df_required.loc[
                    df_required['sub_region_2'] == county
                    ]['sub_region_1'].tolist()
                if self.counties is not None:
                    new_temp_df['county'] = df_required.loc[
                        df_required['sub_region_2'] == county
                        ]['sub_region_2'].tolist()

                # Fill in the dictionary
                new_temp_df['fips'] = fips_list
                new_temp_df['date'] = df_required.loc[
                    df_required['sub_region_2'] == county
                    ]['date'].tolist()
    ...
    # Get the intervention setting dates for the different counties and states
    # in USA
    def get_intervention_data(self):
        ...
        df_data = pd.read_csv(urllib.request.urlopen(
            ...
            "raw/master/complete_npis_inherited_policies.csv"),
            sep=',', error_bad_lines=False)

        # Filter the dataframe based on whether it is required for the counties
        # or states
        if self.counties is not None:
            # Separate case in the case required for all the counties
            if 'all' not in self.counties:
                new_counties = [" ".join(county.split(" ")[:-1])
                                for county in self.counties]
                df_intervention = df_data[
                    df_data['county'].isin(new_counties) &
                    df_data['state'].isin(self.states)
                    ]
            else:
                df_intervention = df_data[
                    df_data['state'].isin(self.states) &
                    df_data['county'].isnull() == False
                    ]
...
def get_data(paramdict):
    ...
    if paramdict['country'] == 'United States' or paramdict['country'] is None:
        ...
        if (paramdict['counties'] is None or 'all' not in paramdict[
            'counties']):
            ...
            # temp_df = pd.concat(
            #     (df_required[columns], df_intervention[columns]),
            #     ignore_index=True)
            ...
            updated_county_list = [unique_comparisons[i][0] for i in
                                   range(len(unique_comparisons))]
            updated_date_list = [unique_comparisons[i][1] for i in
                                 range(len(unique_comparisons))]
            dict_intervention = {
                id_string: updated_county_list,
        ...
        if (paramdict['counties'] is None or
                'all' not in paramdict['counties']):
