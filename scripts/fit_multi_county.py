...
for county_data in counties:
    ...
    # Load Activity Data
    state_path = os.path.join(ROOT_DIR, 'data','Mobility data', '{}_mobility.csv'.format(state_name))
    if os.path.exists(state_path):
      ...
      for category in mkeys:
          ...
          with open(os.path.join(ROOT_DIR, 'data','Mobility data', '{}_mobility.csv'.format(state_name)),'r') as f:
              csv_reader = csv.reader(f, delimiter=',')
              header = next(csv_reader)
              for row in csv_reader:
                  if row[1] == state_name and row[2].replace(' County', '') == county_name and row[3] == category:
                      dates = eval(row[6])
                      vals = eval(row[7])
      ...
      for category in mkeys:
          mobilities[category] = OrderedDict()
          with open(os.path.join(ROOT_DIR, 'data','Mobility data', 'US_mobility.csv'),'r') as f:
              csv_reader = csv.reader(f, delimiter=',')
              header = next(csv_reader)
              for row in csv_reader:
                  if row[1] == 'US' and row[2] == state_name and row[3] == category:
                      dates = eval(row[6])
                      vals = eval(row[7])
                      for i, date in enumerate(dates):
                          mobilities[category][date] = vals[i]
    ...
    def build_model(i0, b_lstm=False, update_k=False):
        ...
        model.add_module("name", sirnet.SEIRNet(i0=i0, b_lstm=b_lstm, update_k=update_k))
        return model
    ...
    for i in range(40,101,2):
      ...
      axs[0].set_title('Quarantine/Normal: {:02d}/{:02d} | Avg. Mobility: {:02d}%'.format(100-i,i,pct))
      axs[0].set_xticklabels([])

      axs[1].plot(days,hospitalized,'b', days,total_deaths, 'k',days, bed40, 'c', bed70,'r')
      axs[1].legend(['Hospitalized','Total Deaths','40% of Hospital Beds', '70% of Hospital Beds'], loc='upper left')
    ...
    plt.legend(pcs, ['Current Mobility', 'Return to Normal Mobility', '50% Return to Normal', '20% Return to Normal',
                     'Available Beds'])
    plt.show()
