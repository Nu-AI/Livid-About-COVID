...
###############################################################################
reporting_rate = 0.50  # Portion of cases that are actually detected
...
for county_data in counties:
    ...
    def build_model(i0, b_lstm=False, update_k=False):
        ...
        model.add_module("name",
                         SIRNet.SIRNet(i0=i0, b_lstm=b_lstm, update_k=update_k))
        return model
    ...
    def train(model, loss, optimizer, x, y):
        ...
        for name, param in model.named_parameters():
            if name != "name.h2o.weight":
                param.data.clamp_(
                    1e-4)  # k and b coefficients cannot be less than 0
