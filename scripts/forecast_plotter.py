...
def get_arrays(dict, scenario_list, population):
    ...
    for scenario in scenario_list:
        ...
        active_cases_obs_arr = avg_arr.item().get('Active Cases (observed)')*population
        hospitalized_arr = avg_arr.item().get('Hospitalized')*population
        Total_cases_obs = avg_arr.item().get('Total Cases (observed)')*population
        Total_cases_lat = avg_arr.item().get('Total Cases (latent)')*population
        Active_cases_lat = avg_arr.item().get('Active Cases (latent)')*population
...
def plot_data(data_list, day_list, legend_list, gridplot):
    ...
    dates = ["2020-02-21","2020-08-21"]   # the range of dates you want the generated list to be
    start, end  =[datetime.strptime(_,"%Y-%m-%d") for _ in dates]
    labels = lambda x,y: list(OrderedDict(((x+ timedelta(_)).strftime(r"%b"), None) for _ in range((y - x).days)).keys())
    ...
    if (gridplot!=1):
        ...
        plt.xticks(range(0, max_days, int(max_days/len(xticklabel))), xticklabel, rotation=xtick_rotation,
                   **tick_font)
        if (log_scale):
            plt.yscale('log')
