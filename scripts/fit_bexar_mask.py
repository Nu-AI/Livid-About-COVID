...
for case in total.keys():
  plt.plot(dates, totals[.1][case], cs[case], linewidth=4.0, label='{}. {}% Mobility'.format(cl[case], case))
  #plt.fill_between(dates, totals[.05][case], totals[.30][case], color=cs[case][0], alpha=.1)
  plt.fill_between(dates, totals[.05][case], totals[.1][case], color=cs[case][0], alpha=.1)
  plt.fill_between(dates, totals[.1][case], totals[.30][case], color=cs[case][0], alpha=.1)
...
for zoom in [True, False]:
  plt.figure(dpi=100, figsize=(16,8))
  for case in total.keys():
    plt.plot(dates, actives[.1][case], cs[case],linewidth=4.0, label='{}. {}% Mobility'.format(cl[case], case))
    #plt.fill_between(dates, actives[.05][case], actives[.30][case], color=cs[case][0], alpha=.1)
    plt.fill_between(dates, actives[.05][case], actives[.1][case], color=cs[case][0], alpha=.1)
    plt.fill_between(dates, actives[.1][case], actives[.30][case], color=cs[case][0], alpha=.1)
    pidx = gt.shape[0]+10 if zoom else np.argmax(actives[.1][case])  # write at 10 days or peak
    if case == 50: pidx += 5
    if zoom:
