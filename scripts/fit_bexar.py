...
k = list(cases.keys())
print('Cases data goes from {} to {}'.format(k[0], k[
    -1]))  # These are the dates we will use to look up mobility data
...
dates = [date0 + dt.timedelta(days=int(d + delay_days + start_model)) for d in
         days]
...
for zoom in [True, False]:
    ...
    for case in total.keys():
        plt.plot(dates, active[case], cs[case], linewidth=4.0,
                 label='{}. {}% Mobility'.format(cl[case], case))
        pidx = gt.shape[0] + 10 if zoom else np.argmax(
            active[case])  # write at 10 days or peak
